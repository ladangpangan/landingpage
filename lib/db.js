...
const DEFAULTS = {
  ...
  heroBadge: 'Produsen Ayam Langsung dari Peternak',
  heroTitle: 'Beli, Bayar, Terima — Ayam Frozen Tanpa Ribet',
  heroSubtitle:
  ...
  heroImage: '/landing/produk-5.jpeg',
...
export async function updateLandingSettings(input) {
  ...
  if (typeof input.heroBadge === 'string') {
    update.heroBadge = input.heroBadge.trim().slice(0, 100)
  }
  if (typeof input.heroTitle === 'string') {
    update.heroTitle = input.heroTitle.trim().slice(0, 200)
  }
  if (typeof input.heroSubtitle === 'string') {
    update.heroSubtitle = input.heroSubtitle.trim().slice(0, 400)
  }
  if (typeof input.heroImage === 'string') {
    update.heroImage = input.heroImage.trim().slice(0, 300)
  }
